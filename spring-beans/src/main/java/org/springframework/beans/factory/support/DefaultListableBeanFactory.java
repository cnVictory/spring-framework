	...
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			...
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				...
				// bean的类型是FactoryBean 进行特殊处理
				if (isFactoryBean(beanName)) {

					// 如果是FactoryBean类型的，给beanName加上&前缀，然后获取FactoryBean的实例，不是getObject返回的实例
					// getBean(beanName) 返回getObject方法创建的对象， getBean(&beanName)返回FactoryBean实例
					...
					if (bean instanceof FactoryBean) {
						...
						if (isEagerInit) {
							...
							// 这里传递进去的是FactoryBean的beanName
							getBean(beanName);
						}
					}
				}

				// 这里传递进去的是非FactoryBean的beanName
				else {
					getBean(beanName);
				}
			}
